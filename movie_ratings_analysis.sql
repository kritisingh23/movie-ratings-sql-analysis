...
(SELECT count(*) FROM genre) as total_genre,
(SELECT count(*) FROM director_mapping) as total_director, 
(SELECT count(*) FROM role_mapping) as total_roles,
...
SELECT
	...
	COUNT(*) - COUNT(worlwide_gross_income) AS null_in_worlwide_gross_income, 
	COUNT(*) - COUNT(languages) AS null_in_languages, 
	COUNT(*) - COUNT(production_company) AS null_in_production_company 
FROM movie;
...
SELECT count(*) as movie_India_USA 
	FROM movie 
...
SELECT count(*) as total_movies, genre 
	FROM genre 
	GROUP BY genre 
...
--SELECT * from genre where movie_id like '%1';
...
SELECT * FROM (
		...
		FROM ratings r
		INNER JOIN movie m
		ON r.movie_id = m.id) AS movie_rank_list
	WHERE movie_rank < = 10;
...
SELECT * FROM(
	...
	GROUP BY production_company) AS prod_company_ranked
...
SELECT * FROM (
	SELECT SUM(r.total_votes) as german_movie_votes
		...
		INNER JOIN ratings r
		ON m.id = r.movie_id
		WHERE m.country = 'Germany') as german
CROSS JOIN (
	SELECT SUM(r.total_votes) as italian_movie_votes
		FROM movie m
		INNER JOIN ratings r
		ON m.id = r.movie_id
		WHERE m.country = 'Italy') as italy;
...
--Common table expression to find top 3 genre based on movie count and having avg_rating > 8
WITH top_genre AS(
	SELECT TOP 3 g.genre, count(*) AS movie_count
	FROM genre g
	LEFT JOIN ratings r
...
--Common table expression to find directors with their movie_count
director_movie_count AS(
    ...
    WHERE g.genre IN (SELECT genre FROM top_genre) AND r.avg_rating > 8
    GROUP BY n.name)
...
SELECT TOP 3 * FROM (
	...
	FROM movie m
	INNER JOIN ratings r ON m.id = r.movie_id
	GROUP BY m.production_company) as prod_rank;
...
--CTE for finding total votes and wighted average rating for actors having acted in at least five Indian movies
actor_info AS (
	SELECT
        name_id,
        COUNT(movie_id) AS movie_count,
        SUM(avg_rating * total_votes) / SUM(total_votes) AS weighted_avg_rating,
...
SELECT 
	...
	total_votes, 
	movie_count,
    weighted_avg_rating as actor_avg_rating,
...
SELECT genre, avg_duration,
    ...
    -- Running total of avg duration
    SUM(avg_duration) OVER (
        ORDER BY avg_duration
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
    ) AS running_total_duration,

    -- Moving average of avg duration
...
SELECT * FROM (
	...
	WHERE genre in (SELECT genre from top_genre)) AS movie_ranks
	WHERE movie_rank <= 5;
...
SELECT TOP 2 production_company, 
	count(*) as movie_count,
	RANK () OVER (ORDER BY count(*) DESC) as prod_comp_rank
	FROM movie m
	INNER JOIN ratings r ON m.id = r.movie_id
	WHERE languages LIKE '%,%' AND r.median_rating >= 8 AND production_company IS NOT NULL
...
--CTE for Superhit drama category
WITH superhit_drama AS (
	SELECT * FROM (
	 SELECT 
		...
		INNER JOIN ratings r ON m.id = r.movie_id
		INNER JOIN genre g ON m.id = g.movie_id
		WHERE g.genre = 'Drama') as drama_categories
		WHERE movie_category = 'Superhit')

SELECT TOP 3 n.name as actress_name, 
	...
	n.name ASC) as actress_rank 

	FROM names n
	JOIN role_mapping rm ON n.id = rm.name_id
	JOIN movie m ON rm.movie_id = m.id
	JOIN ratings r ON m.id = r.movie_id
	WHERE m.title in (SELECT movie_name FROM superhit_drama) AND rm.category = 'actress'
	GROUP BY n.name;
...
SELECT TOP 9
	...
	SUM(m.duration) AS totoal_duration
FROM names n 
